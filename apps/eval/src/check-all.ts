...
for (const e of evts.slice(0, 5)) {
  ...
  console.log(`  [${e.type}] ${d.method ?? d.level ?? ""} ${d.url ?? (d.args as any)?.[0] ?? ""} trace=${e.traceId ?? "none"}`)
}

const snaps = await db.select().from(stateSnapshots).orderBy(desc(stateSnapshots.timestamp))
console.log(`\n=== ${snaps.length} state snapshots ===`)
for (const s of snaps) {
  const type = s.value ? "full" : "diff"
  const data = s.value ?? s.diff
  console.log(`  [${s.key}] (${type}) ${JSON.stringify(data)} trace=${(s as any).traceId ?? "none"}`)
}

const fns = await db.select().from(functionCalls).orderBy(desc(functionCalls.timestamp))
console.log(`\n=== ${fns.length} function calls ===`)
for (const f of fns) {
  console.log(`  ${f.name}(${JSON.stringify(f.args)}) → ${JSON.stringify(f.returnValue)} ${f.error ? `ERR: ${f.error}` : ""} [${f.durationMs?.toFixed(2)}ms] trace=${(f as any).traceId ?? "none"}`)
}
