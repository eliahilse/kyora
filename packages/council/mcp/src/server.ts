...
/** A job that outlives its own run timeout can only be a casualty of a restart. */
async function deadlineFor(cwd: string, startedAt: number): Promise<number> {
  return startedAt + (await runConfig(cwd)).timeoutMs + 60_000
}
...
server.tool(
  ...
  async ({ question, context, engines, size, effort, cwd }) => {
    const root = cwdOf(cwd)
    const { seated, skipped } = await seatCouncil({ engines, size, cwd: root })
    if (seated.length === 0) {
      return text(
        `No council member could be seated — every engine is unavailable, cooling down, or out of quota. Skipped: ${skipped.join(", ")}. Proceed on your own judgment, and say so.`,
      )
    }
    return text(summarizeReplies(await convene({ question, context, seated, effort, cwd: root })))
  },
)
...
server.tool(
  ...
  async ({ question, context, engines, size, cwd }) => {
    ...
    const { seated } = await seatCouncil({ engines, size, cwd: root })
    if (seated.length === 0) return text(`Council ${id} could not seat any member (no engine has quota right now).`)
    const job: Job = {
      id,
      kind: "council",
      status: "running",
      question,
      engines: seated.map((engine) => engine.id),
      startedAt: started,
      deadlineAt: await deadlineFor(root, started),
    }
    saveJob(job)
    void convene({ question, context, seated, cwd: root })
      .then((replies) => saveJob({ ...job, status: "done", finishedAt: Date.now(), replies }))
      .catch((error: unknown) =>
...
server.tool(
  ...
  async ({ engine, question, context, model, effort, cwd }) => {
    ...
    const seated = await healthyEngines([engine], root)
    if (seated.length === 0) return text(`${engine} is unavailable, cooling down, or out of quota right now.`)
    const reply = await askEngine(seated[0]!, askPrompt(question, context), root, {
      ...(model ? { model } : {}),
      ...(effort ? { effort } : {}),
    })
...
server.tool(
  ...
  async ({ engine, task, context, model, effort, write, background, cwd }) => {
    ...
    const seated = await healthyEngines([engine], root, mode)
    if (seated.length === 0) {
      const def = engineById(engine)
    ...
    if (!background) {
      const reply = await askEngine(seated[0]!, prompt, root, runOptions)
      return text(reply.ok ? reply.text : `${engine} failed: ${reply.text}`)
    }
    const started = Date.now()
    ...
    saveJob(job)
    void askEngine(seated[0]!, prompt, root, runOptions)
...
server.tool(
  ...
  async ({ task, context, prefer, model, effort, write, background, cwd }) => {
    const root = cwdOf(cwd)
    const mode = write ? ("write" as const) : ("chat" as const)
    const engine = await pickEngine(prefer, mode, root)
    if (!engine) {
      return text(
    ...
    if (!background) {
      const reply = await askEngine(engine, prompt, root, runOptions)
    ...
    saveJob(job)
    void askEngine(engine, prompt, root, runOptions)
      .then((reply) => saveJob({ ...job, status: "done", finishedAt: Date.now(), replies: [reply] }))
      .catch((error: unknown) => saveJob({ ...job, status: "failed", finishedAt: Date.now(), error: String(error) }))
...
server.tool(
  ...
  "Run several DIFFERENT tasks in parallel, each on a different model family. Use to parallelize independent workstreams across subscriptions — not to ask one question many ways (that is council_convene).",
  {
    ...
    write: z
      .boolean()
      .optional()
      .describe("allow file edits — write tasks share one checkout, so they are run one after another, not in parallel"),
    cwd: z.string().optional(),
  },
  async ({ tasks, write, cwd }) => {
    if (tasks.length === 0) return text("No tasks given.")
    const { results, unassigned, serialized } = await fanout({ tasks, write, cwd: cwdOf(cwd) })
    ...
    if (serialized) {
      blocks.push(
        "### note\n\nWrite tasks ran sequentially: they share one working tree, and parallel editors would overwrite each other.",
      )
    }
...
server.tool(
  ...
  { cwd: z.string().optional() },
  async ({ cwd }) => {
    const lines = (await engineCatalog(cwdOf(cwd))).map((entry) => {
...
server.tool(
  ...
  { cwd: z.string().optional() },
  async ({ cwd }) => {
    const health = await councilStatus(cwdOf(cwd))
